 ...
 */
package me.xiaokui.modules.system.repository;
...
public interface UserRepository extends JpaRepository<User, Long>, JpaSpecificationExecutor<User> {
     ...
     * @return /
     */
    @Query(value = "SELECT u.* FROM sys_user u, sys_users_roles r, sys_roles_projects p WHERE " +
            "u.user_id = r.user_id AND r.role_id = p.role_id AND r.role_id = ?1 group by u.user_id", nativeQuery = true)
    List<User> findByProjectRoleId(Long id);
     ...
     * @return /
     */
    @Query(value = "SELECT count(1) FROM sys_user u WHERE u.project_id IN ?1", nativeQuery = true)
    int countByProjects(Set<Long> projectIds);
